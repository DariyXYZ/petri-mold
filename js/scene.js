      ...
      if (o) {
        ...
        if (c) {
          ...
          if (c.a.hollow && !edge && !seam) { /* оставляем агар */ }
            ...
            // Резкая граница читалась как вырезанная фигура. Своего значения
            // нет — берётся короткий фейд по умолчанию: ни одна клетка не
            // должна включаться на полную яркость в один тик.
            var ef = c.a.edgeFade === undefined ? 26 : c.a.edgeFade;
            if (ef) {
              var op = PM.rng.smoothstep(0, ef, since);
              l = lum[i] * (1 - op) + l * op;
            }
